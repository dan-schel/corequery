...
export class QuasilinearStopDiagramCanvasController extends CanvasController<QuasilinearStopDiagramCanvasData> {
  ...
  override onRender(): void {
    if (this.data.structure.type !== "linear") return;
    if (this.data.structure.stops.length === 0) return;

    const labelYLevels = this._getLabelYLevels();
    if (labelYLevels.length !== this.data.structure.stops.length) return;
    ...
    this.data.structure.stops.forEach((stop, index) => {
      const labelY = itsOk(labelYLevels[index]);

      const fullSize =
        index === 0 || index === this.data.structure.stops.length - 1;

      if (stop.drawMark !== false) {
    ...
    return Array.from(this.data.labelsParent?.childNodes.values() ?? [])
      ...
      .map(
        (label) =>
          ...
          this._canvasContainer.offsetTop,
      );
  }
}
